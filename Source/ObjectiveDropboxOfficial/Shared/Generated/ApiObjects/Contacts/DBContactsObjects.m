...
- (instancetype)initWithEmailAddresses:(NSArray<NSString *> *)emailAddresses {
    [DBStoneValidators nonnullValidator:[DBStoneValidators arrayValidator:nil maxItems:nil itemValidator:[DBStoneValidators nonnullValidator:[DBStoneValidators stringValidator:nil maxLength:@(255) pattern:@"^['#&A-Za-z0-9._%+-]+@[A-Za-z0-9-][A-Za-z0-9.-]*\\.[A-Za-z]{2,15}$"]]]](emailAddresses);
...
+ (NSDictionary<NSString *, id> *)serialize:(DBCONTACTSDeleteManualContactsArg *)valueObj {
    ...
    jsonDict[@"email_addresses"] = [DBArraySerializer serialize:valueObj.emailAddresses withBlock:^id(id elem0) { return elem0; }];

    return jsonDict;
}

+ (DBCONTACTSDeleteManualContactsArg *)deserialize:(NSDictionary<NSString *, id> *)valueDict {
    NSArray<NSString *> *emailAddresses = [DBArraySerializer deserialize:valueDict[@"email_addresses"] withBlock:^id(id elem0) { return elem0; }];
...
- (NSArray<NSString *> *)contactsNotFound {
    if (![self isContactsNotFound]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBCONTACTSDeleteManualContactsErrorContactsNotFound, but was %@.", [self tagName]];
    }
...
+ (NSDictionary<NSString *, id> *)serialize:(DBCONTACTSDeleteManualContactsError *)valueObj {
    ...
    if ([valueObj isContactsNotFound]) {
        jsonDict[@"contacts_not_found"] = [DBArraySerializer serialize:valueObj.contactsNotFound withBlock:^id(id elem0) { return elem0; }];
...
+ (DBCONTACTSDeleteManualContactsError *)deserialize:(NSDictionary<NSString *, id> *)valueDict {
    ...
    if ([tag isEqualToString:@"contacts_not_found"]) {
        NSArray<NSString *> *contactsNotFound = [DBArraySerializer deserialize:valueDict[@"contacts_not_found"] withBlock:^id(id elem0) { return elem0; }];
