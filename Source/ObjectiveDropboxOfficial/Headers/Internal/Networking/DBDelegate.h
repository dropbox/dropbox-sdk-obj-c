...
///
/// By default, this delegate is instantiated in the constructor of the `DBTransportDefaultClient` class, and uses the main delegate queue, so all handler code will be executed serially and on the main thread.
///
/// Progress and response handlers can be added after the request is initiated. If a handler does not exist at the time the network response is received, progress data and/or response data will be saved until a handler is queued up to the corresponding task ID. For downloaded file content, the file content will be moved from an `NSURLSession` managed temporary location to an SDK managed temporary location, until the response handler is installed, at which point, the file content will be moved to the final destination. This gives the client the flexibility to install handlers when convenient.
///
...
///
/// @param delegateQueue The queue used to execute handler code. By defaut, this is the main queue, so all handler code will be executed on the main thread.
///
...
///
/// Enqueues a handler to be executed periodically to retrieve information on the progress of the supplied `NSURLSessionTask` task for the corresponding Upload-style request.
///
