...
///
/// BETA: Batch uploads small and large files.
///
...
///
/// @note Currently in BETA. The interface route does not have the same structure as other routes in the SDK.
/// Here, no request object is returned. Instead, progress and response handlers are passed
/// in directly to the route, rather than installed via the response object.
///
...
///
- (void)batchUploadFiles:(NSDictionary<NSURL *, DBFILESCommitInfo *> * _Nonnull)fileUrlsToCommitInfo
...
@property (nonatomic) NSOperationQueue * _Nonnull queue;
...
@property (nonatomic) dispatch_group_t _Nonnull uploadGroup;

/// A client-supplied parameter that maps the file urls of the files to upload
/// to the corresponding commit info objects.
@property (nonatomic) NSDictionary<NSURL *, DBFILESCommitInfo *> * _Nonnull fileUrlsToCommitInfo;
...
@property (nonatomic) NSMutableArray<DBFILESUploadSessionFinishArg *> * _Nonnull finishArgs;

/// The progress block that is periodically executed once a file upload is complete.
@property (nonatomic) DBProgressBlock _Nullable progressBlock;

/// The response block that is executed once all file uploads and the final batch commit
/// is complete.
@property (nonatomic) DBBatchUploadResponseBlock _Nonnull responseBlock;
