...
- (DBAccessToken *)accessToken {
    if (_tag != DBAuthSuccess) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required `DBAuthSuccess`, but was %@.", [self tagName]];
    }
    return _accessToken;
}
...
- (DBAuthErrorType)errorType {
    if (_tag != DBAuthError) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required `DBAuthError`, but was %@.", [self tagName]];
    }
    return _errorType;
}
