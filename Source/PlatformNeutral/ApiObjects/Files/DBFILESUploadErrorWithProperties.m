...
- (DBFILESUploadWriteFailed *)path {
    if (![self isPath]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBFILESUploadErrorWithPropertiesPath, but was %@.", [self tagName]];
    }
    return _path;
}
...
- (DBFILESInvalidPropertyGroupError *)propertiesError {
    if (![self isPropertiesError]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBFILESUploadErrorWithPropertiesPropertiesError, but was %@.", [self tagName]];
    }
...
+ (NSDictionary *)serialize:(DBFILESUploadErrorWithProperties *)valueObj {
    ...
    if ([valueObj isPath]) {
        ...
        jsonDict = [[DBFILESInvalidPropertyGroupErrorSerializer serialize:valueObj.propertiesError] mutableCopy];
...
+ (DBFILESUploadErrorWithProperties *)deserialize:(NSDictionary *)valueDict {
    ...
    if ([tag isEqualToString:@"path"]) {
        ...
        DBFILESInvalidPropertyGroupError *propertiesError = [DBFILESInvalidPropertyGroupErrorSerializer deserialize:valueDict[@"properties_error"]];
