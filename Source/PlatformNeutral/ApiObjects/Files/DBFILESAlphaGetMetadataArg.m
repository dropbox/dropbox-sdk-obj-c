...
+ (NSDictionary *)serialize:(DBFILESAlphaGetMetadataArg *)valueObj {
    ...
    if (valueObj.includePropertyTemplates) {
        jsonDict[@"include_property_templates"] = [DBArraySerializer serialize:valueObj.includePropertyTemplates withBlock:^id(id elem) { return elem; }];
    }
...
+ (DBFILESAlphaGetMetadataArg *)deserialize:(NSDictionary *)valueDict {
    ...
    NSArray<NSString *> *includePropertyTemplates = valueDict[@"include_property_templates"] ? [DBArraySerializer deserialize:valueDict[@"include_property_templates"] withBlock:^id(id elem) { return elem; }] : nil;

    return [[DBFILESAlphaGetMetadataArg alloc] initWithPath:path includeMediaInfo:includeMediaInfo includeDeleted:includeDeleted includeHasExplicitSharedMembers:includeHasExplicitSharedMembers includePropertyTemplates:includePropertyTemplates];
}
