...
- (NSString *)templateNotFound {
    if (![self isTemplateNotFound]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBFILESPropertiesErrorTemplateNotFound, but was %@.", [self tagName]];
    }
...
- (DBFILESLookupError *)path {
    if (![self isPath]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBFILESPropertiesErrorPath, but was %@.", [self tagName]];
    }
    return _path;
}
