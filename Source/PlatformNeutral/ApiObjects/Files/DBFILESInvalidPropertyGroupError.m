...
- (NSString *)templateNotFound {
    if (![self isTemplateNotFound]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBFILESInvalidPropertyGroupErrorTemplateNotFound, but was %@.", [self tagName]];
    }
...
- (DBFILESLookupError *)path {
    if (![self isPath]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBFILESInvalidPropertyGroupErrorPath, but was %@.", [self tagName]];
    }
    return _path;
}
