...
- (DBSHARINGSharingUserError *)userError {
    if (![self isUserError]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGRemoveFileMemberErrorUserError, but was %@.", [self tagName]];
    }
    return _userError;
}
...
- (DBSHARINGSharingFileAccessError *)accessError {
    if (![self isAccessError]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGRemoveFileMemberErrorAccessError, but was %@.", [self tagName]];
    }
    return _accessError;
}
...
- (DBSHARINGMemberAccessLevelResult *)noExplicitAccess {
    if (![self isNoExplicitAccess]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGRemoveFileMemberErrorNoExplicitAccess, but was %@.", [self tagName]];
    }
...
+ (NSDictionary *)serialize:(DBSHARINGRemoveFileMemberError *)valueObj {
    ...
    if ([valueObj isUserError]) {
        jsonDict = [[DBSHARINGSharingUserErrorSerializer serialize:valueObj.userError] mutableCopy];
        jsonDict[@".tag"] = @"user_error";
    }
    else if ([valueObj isAccessError]) {
        jsonDict = [[DBSHARINGSharingFileAccessErrorSerializer serialize:valueObj.accessError] mutableCopy];
...
+ (DBSHARINGRemoveFileMemberError *)deserialize:(NSDictionary *)valueDict {
    ...
    if ([tag isEqualToString:@"user_error"]) {
        ...
        DBSHARINGSharingFileAccessError *accessError = [DBSHARINGSharingFileAccessErrorSerializer deserialize:valueDict[@"access_error"]];
        ...
        DBSHARINGMemberAccessLevelResult *noExplicitAccess = [DBSHARINGMemberAccessLevelResultSerializer deserialize:valueDict];
