...
- (DBSHARINGMemberAccessLevelResult *)complete {
    if (![self isComplete]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGRemoveMemberJobStatusComplete, but was %@.", [self tagName]];
    }
    return _complete;
}
...
- (DBSHARINGRemoveFolderMemberError *)failed {
    if (![self isFailed]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGRemoveMemberJobStatusFailed, but was %@.", [self tagName]];
    }
    return _failed;
}
...
+ (NSDictionary *)serialize:(DBSHARINGRemoveMemberJobStatus *)valueObj {
    ...
    if ([valueObj isInProgress]) {
        ...
        jsonDict = [[DBSHARINGRemoveFolderMemberErrorSerializer serialize:valueObj.failed] mutableCopy];
        jsonDict[@".tag"] = @"failed";
    }
    else {
        @throw([NSException exceptionWithName:@"InvalidTag" reason:@"Tag has an invalid value." userInfo:nil]);
...
+ (DBSHARINGRemoveMemberJobStatus *)deserialize:(NSDictionary *)valueDict {
    ...
    if ([tag isEqualToString:@"in_progress"]) {
        ...
        DBSHARINGRemoveFolderMemberError *failed = [DBSHARINGRemoveFolderMemberErrorSerializer deserialize:valueDict[@"failed"]];
