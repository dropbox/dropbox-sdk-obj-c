...
- (DBSHARINGSharedFolderAccessError *)accessError {
    if (![self isAccessError]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGAddFolderMemberErrorAccessError, but was %@.", [self tagName]];
    }
    return _accessError;
}
...
- (DBSHARINGAddMemberSelectorError *)badMember {
    if (![self isBadMember]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGAddFolderMemberErrorBadMember, but was %@.", [self tagName]];
    }
    return _badMember;
}
...
- (NSNumber *)tooManyMembers {
    if (![self isTooManyMembers]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGAddFolderMemberErrorTooManyMembers, but was %@.", [self tagName]];
    }
...
- (NSNumber *)tooManyPendingInvites {
    if (![self isTooManyPendingInvites]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGAddFolderMemberErrorTooManyPendingInvites, but was %@.", [self tagName]];
    }
...
+ (NSDictionary *)serialize:(DBSHARINGAddFolderMemberError *)valueObj {
    ...
    if ([valueObj isAccessError]) {
        ...
        jsonDict = [[DBSHARINGAddMemberSelectorErrorSerializer serialize:valueObj.badMember] mutableCopy];
...
+ (DBSHARINGAddFolderMemberError *)deserialize:(NSDictionary *)valueDict {
    ...
    if ([tag isEqualToString:@"access_error"]) {
        ...
        DBSHARINGAddMemberSelectorError *badMember = [DBSHARINGAddMemberSelectorErrorSerializer deserialize:valueDict[@"bad_member"]];
