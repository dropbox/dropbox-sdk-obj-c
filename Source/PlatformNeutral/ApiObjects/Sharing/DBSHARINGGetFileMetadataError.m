...
- (DBSHARINGSharingUserError *)userError {
    if (![self isUserError]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGGetFileMetadataErrorUserError, but was %@.", [self tagName]];
    }
    return _userError;
}
...
- (DBSHARINGSharingFileAccessError *)accessError {
    if (![self isAccessError]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGGetFileMetadataErrorAccessError, but was %@.", [self tagName]];
    }
    return _accessError;
}
