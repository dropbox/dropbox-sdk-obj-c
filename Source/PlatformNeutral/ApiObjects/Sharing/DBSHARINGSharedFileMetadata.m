...
- (instancetype)initWithPolicy:(DBSHARINGFolderPolicy *)policy previewUrl:(NSString *)previewUrl name:(NSString *)name id_:(NSString *)id_ permissions:(NSArray<DBSHARINGFilePermission *> *)permissions ownerTeam:(DBUSERSTeam *)ownerTeam parentSharedFolderId:(NSString *)parentSharedFolderId pathLower:(NSString *)pathLower pathDisplay:(NSString *)pathDisplay {
    [DBStoneValidators stringValidator:@(1) maxLength:nil pattern:@"id:.*"](id_);
    [DBStoneValidators nullableValidator:[DBStoneValidators arrayValidator:nil maxItems:nil itemValidator:nil]](permissions);
...
- (instancetype)initWithPolicy:(DBSHARINGFolderPolicy *)policy previewUrl:(NSString *)previewUrl name:(NSString *)name id_:(NSString *)id_ {
...
+ (NSDictionary *)serialize:(DBSHARINGSharedFileMetadata *)valueObj {
    ...
    if (valueObj.permissions) {
        jsonDict[@"permissions"] = [DBArraySerializer serialize:valueObj.permissions withBlock:^id(id elem) { return [DBSHARINGFilePermissionSerializer serialize:elem]; }];
    }
...
+ (DBSHARINGSharedFileMetadata *)deserialize:(NSDictionary *)valueDict {
    ...
    NSArray<DBSHARINGFilePermission *> *permissions = valueDict[@"permissions"] ? [DBArraySerializer deserialize:valueDict[@"permissions"] withBlock:^id(id elem) { return [DBSHARINGFilePermissionSerializer deserialize:elem]; }] : nil;
    DBUSERSTeam *ownerTeam = valueDict[@"owner_team"] ? [DBUSERSTeamSerializer deserialize:valueDict[@"owner_team"]] : nil;
    ...
    return [[DBSHARINGSharedFileMetadata alloc] initWithPolicy:policy previewUrl:previewUrl name:name id_:id_ permissions:permissions ownerTeam:ownerTeam parentSharedFolderId:parentSharedFolderId pathLower:pathLower pathDisplay:pathDisplay];
}
