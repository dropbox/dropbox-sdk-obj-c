...
+ (NSDictionary *)serialize:(DBSHARINGListFilesResult *)valueObj {
    ...
    jsonDict[@"entries"] = [DBArraySerializer serialize:valueObj.entries withBlock:^id(id elem) { return [DBSHARINGSharedFileMetadataSerializer serialize:elem]; }];
    if (valueObj.cursor) {
...
+ (DBSHARINGListFilesResult *)deserialize:(NSDictionary *)valueDict {
    NSArray<DBSHARINGSharedFileMetadata *> *entries = [DBArraySerializer deserialize:valueDict[@"entries"] withBlock:^id(id elem) { return [DBSHARINGSharedFileMetadataSerializer deserialize:elem]; }];
