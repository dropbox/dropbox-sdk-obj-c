...
- (DBSHARINGAccessLevel *)success {
    if (![self isSuccess]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGFileMemberActionIndividualResultSuccess, but was %@.", [self tagName]];
    }
    return _success;
}
...
- (DBSHARINGFileMemberActionError *)memberError {
    if (![self isMemberError]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGFileMemberActionIndividualResultMemberError, but was %@.", [self tagName]];
    }
    return _memberError;
}
...
+ (NSDictionary *)serialize:(DBSHARINGFileMemberActionIndividualResult *)valueObj {
    ...
    if ([valueObj isSuccess]) {
        ...
        jsonDict = [[DBSHARINGFileMemberActionErrorSerializer serialize:valueObj.memberError] mutableCopy];
...
+ (DBSHARINGFileMemberActionIndividualResult *)deserialize:(NSDictionary *)valueDict {
    ...
    if ([tag isEqualToString:@"success"]) {
        DBSHARINGAccessLevel *success = valueDict[@"success"] ? [DBSHARINGAccessLevelSerializer deserialize:valueDict[@"success"]] : nil;
        return [[DBSHARINGFileMemberActionIndividualResult alloc] initWithSuccess:success];
    }
    else if ([tag isEqualToString:@"member_error"]) {
        DBSHARINGFileMemberActionError *memberError = [DBSHARINGFileMemberActionErrorSerializer deserialize:valueDict[@"member_error"]];
