...
/// (in case the link's owner is part of a team) and the shared folder (in case the linked file is
/// part of a shared folder). This field is shown only if the caller has access to this info (the
...
/// resolvedVisibility. This is shown only if the caller is the link's owner.
...
///
/// Full constructor for the DBSHARINGLinkPermissions struct (exposes all instance variables).
///
/// @param canRevoke Whether the caller can revoke the shared link
/// @param resolvedVisibility The current visibility of the link after considering the shared links
/// policies of the the team (in case the link's owner is part of a team) and the shared folder (in
/// case the linked file is part of a shared folder). This field is shown only if the caller has
/// access to this info (the link's owner always has access to this data).
/// @param requestedVisibility The shared link's requested visibility. This can be overridden by the
/// team and shared folder policies. The final visibility, after considering these policies, can be
/// found in :field:`resolved_visibility`. This is shown only if the caller is the link's owner.
/// @param revokeFailureReason The failure reason for revoking the link. This field will only be
/// present if the :field:`can_revoke` is :val:`false`.
///
/// @return An initialized DBSHARINGLinkPermissions instance.
///
...
///
/// Convenience constructor for the DBSHARINGLinkPermissions struct (exposes only non-nullable
...
///
/// @return A json-compatible dictionary representation of the DBSHARINGLinkPermissions API object.
///
...
///
/// @param dict A json-compatible dictionary representation of the DBSHARINGLinkPermissions API
/// object.
///
