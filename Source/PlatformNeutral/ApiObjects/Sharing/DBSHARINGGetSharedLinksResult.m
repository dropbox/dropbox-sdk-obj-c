...
+ (NSDictionary *)serialize:(DBSHARINGGetSharedLinksResult *)valueObj {
    ...
    jsonDict[@"links"] = [DBArraySerializer serialize:valueObj.links withBlock:^id(id elem) { return [DBSHARINGLinkMetadataSerializer serialize:elem]; }];

    return jsonDict;
}

+ (DBSHARINGGetSharedLinksResult *)deserialize:(NSDictionary *)valueDict {
    NSArray<DBSHARINGLinkMetadata *> *links = [DBArraySerializer deserialize:valueDict[@"links"] withBlock:^id(id elem) { return [DBSHARINGLinkMetadataSerializer deserialize:elem]; }];
