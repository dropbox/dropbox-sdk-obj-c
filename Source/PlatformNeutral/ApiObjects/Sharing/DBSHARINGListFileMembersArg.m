...
- (instancetype)initWithFile:(NSString *)file actions:(NSArray<DBSHARINGMemberAction *> *)actions includeInherited:(NSNumber *)includeInherited limit:(NSNumber *)limit {
...
+ (NSDictionary *)serialize:(DBSHARINGListFileMembersArg *)valueObj {
    ...
    if (valueObj.actions) {
        jsonDict[@"actions"] = [DBArraySerializer serialize:valueObj.actions withBlock:^id(id elem) { return [DBSHARINGMemberActionSerializer serialize:elem]; }];
    }
...
+ (DBSHARINGListFileMembersArg *)deserialize:(NSDictionary *)valueDict {
    ...
    NSArray<DBSHARINGMemberAction *> *actions = valueDict[@"actions"] ? [DBArraySerializer deserialize:valueDict[@"actions"] withBlock:^id(id elem) { return [DBSHARINGMemberActionSerializer deserialize:elem]; }] : nil;
    NSNumber *includeInherited = valueDict[@"include_inherited"];
    NSNumber *limit = valueDict[@"limit"];

    return [[DBSHARINGListFileMembersArg alloc] initWithFile:file actions:actions includeInherited:includeInherited limit:limit];
}
