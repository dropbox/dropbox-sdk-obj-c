...
- (DBSHARINGSharedFolderAccessError *)accessError {
    if (![self isAccessError]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGMountFolderErrorAccessError, but was %@.", [self tagName]];
    }
    return _accessError;
}
...
- (DBSHARINGInsufficientQuotaAmounts *)insufficientQuota {
    if (![self isInsufficientQuota]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBSHARINGMountFolderErrorInsufficientQuota, but was %@.", [self tagName]];
    }
...
+ (DBSHARINGMountFolderError *)deserialize:(NSDictionary *)valueDict {
    ...
    if ([tag isEqualToString:@"access_error"]) {
        ...
        DBSHARINGInsufficientQuotaAmounts *insufficientQuota = [DBSHARINGInsufficientQuotaAmountsSerializer deserialize:valueDict];
