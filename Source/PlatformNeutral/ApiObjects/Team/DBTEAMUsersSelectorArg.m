...
- (NSArray<NSString *> *)teamMemberIds {
    if (![self isTeamMemberIds]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBTEAMUsersSelectorArgTeamMemberIds, but was %@.", [self tagName]];
    }
...
- (NSArray<NSString *> *)externalIds {
    if (![self isExternalIds]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBTEAMUsersSelectorArgExternalIds, but was %@.", [self tagName]];
    }
    return _externalIds;
}
...
- (NSArray<NSString *> *)emails {
    if (![self isEmails]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBTEAMUsersSelectorArgEmails, but was %@.", [self tagName]];
    }
    return _emails;
}
...
+ (NSDictionary *)serialize:(DBTEAMUsersSelectorArg *)valueObj {
    ...
    if ([valueObj isTeamMemberIds]) {
        jsonDict[@"team_member_ids"] = [DBArraySerializer serialize:valueObj.teamMemberIds withBlock:^id(id elem) { return elem; }];
        jsonDict[@".tag"] = @"team_member_ids";
    }
    else if ([valueObj isExternalIds]) {
        jsonDict[@"external_ids"] = [DBArraySerializer serialize:valueObj.externalIds withBlock:^id(id elem) { return elem; }];
        jsonDict[@".tag"] = @"external_ids";
    }
    else if ([valueObj isEmails]) {
        jsonDict[@"emails"] = [DBArraySerializer serialize:valueObj.emails withBlock:^id(id elem) { return elem; }];
        jsonDict[@".tag"] = @"emails";
    }
    else {
        @throw([NSException exceptionWithName:@"InvalidTag" reason:@"Tag has an invalid value." userInfo:nil]);
...
+ (DBTEAMUsersSelectorArg *)deserialize:(NSDictionary *)valueDict {
    ...
    if ([tag isEqualToString:@"team_member_ids"]) {
        NSArray<NSString *> *teamMemberIds = [DBArraySerializer deserialize:valueDict[@"team_member_ids"] withBlock:^id(id elem) { return elem; }];
        return [[DBTEAMUsersSelectorArg alloc] initWithTeamMemberIds:teamMemberIds];
    }
    else if ([tag isEqualToString:@"external_ids"]) {
        NSArray<NSString *> *externalIds = [DBArraySerializer deserialize:valueDict[@"external_ids"] withBlock:^id(id elem) { return elem; }];
        return [[DBTEAMUsersSelectorArg alloc] initWithExternalIds:externalIds];
    }
    else if ([tag isEqualToString:@"emails"]) {
        NSArray<NSString *> *emails = [DBArraySerializer deserialize:valueDict[@"emails"] withBlock:^id(id elem) { return elem; }];
