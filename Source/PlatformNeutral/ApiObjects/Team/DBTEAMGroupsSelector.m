...
- (NSArray<NSString *> *)groupIds {
    if (![self isGroupIds]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBTEAMGroupsSelectorGroupIds, but was %@.", [self tagName]];
    }
    return _groupIds;
}
...
- (NSArray<NSString *> *)groupExternalIds {
    if (![self isGroupExternalIds]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBTEAMGroupsSelectorGroupExternalIds, but was %@.", [self tagName]];
    }
...
+ (NSDictionary *)serialize:(DBTEAMGroupsSelector *)valueObj {
    ...
    if ([valueObj isGroupIds]) {
        jsonDict[@"group_ids"] = [DBArraySerializer serialize:valueObj.groupIds withBlock:^id(id elem) { return elem; }];
        jsonDict[@".tag"] = @"group_ids";
    }
    else if ([valueObj isGroupExternalIds]) {
        jsonDict[@"group_external_ids"] = [DBArraySerializer serialize:valueObj.groupExternalIds withBlock:^id(id elem) { return elem; }];
...
+ (DBTEAMGroupsSelector *)deserialize:(NSDictionary *)valueDict {
    ...
    if ([tag isEqualToString:@"group_ids"]) {
        NSArray<NSString *> *groupIds = [DBArraySerializer deserialize:valueDict[@"group_ids"] withBlock:^id(id elem) { return elem; }];
        return [[DBTEAMGroupsSelector alloc] initWithGroupIds:groupIds];
    }
    else if ([tag isEqualToString:@"group_external_ids"]) {
        NSArray<NSString *> *groupExternalIds = [DBArraySerializer deserialize:valueDict[@"group_external_ids"] withBlock:^id(id elem) { return elem; }];
