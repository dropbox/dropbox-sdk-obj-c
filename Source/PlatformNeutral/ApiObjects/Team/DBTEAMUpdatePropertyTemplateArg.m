...
- (instancetype)initWithTemplateId:(NSString *)templateId name:(NSString *)name description_:(NSString *)description_ addFields:(NSArray<DBPROPERTIESPropertyFieldTemplate *> *)addFields {
    [DBStoneValidators stringValidator:@(1) maxLength:nil pattern:@"(/|ptid:).*"](templateId);
    [DBStoneValidators nullableValidator:[DBStoneValidators arrayValidator:nil maxItems:nil itemValidator:nil]](addFields);
...
+ (NSDictionary *)serialize:(DBTEAMUpdatePropertyTemplateArg *)valueObj {
    ...
    if (valueObj.addFields) {
        jsonDict[@"add_fields"] = [DBArraySerializer serialize:valueObj.addFields withBlock:^id(id elem) { return [DBPROPERTIESPropertyFieldTemplateSerializer serialize:elem]; }];
    }
...
+ (DBTEAMUpdatePropertyTemplateArg *)deserialize:(NSDictionary *)valueDict {
    ...
    NSArray<DBPROPERTIESPropertyFieldTemplate *> *addFields = valueDict[@"add_fields"] ? [DBArraySerializer deserialize:valueDict[@"add_fields"] withBlock:^id(id elem) { return [DBPROPERTIESPropertyFieldTemplateSerializer deserialize:elem]; }] : nil;

    return [[DBTEAMUpdatePropertyTemplateArg alloc] initWithTemplateId:templateId name:name description_:description_ addFields:addFields];
}
