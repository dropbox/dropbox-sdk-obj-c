...
- (NSString *)asyncJobId {
    if (![self isAsyncJobId]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBTEAMMembersAddLaunchAsyncJobId, but was %@.", [self tagName]];
    }
    return _asyncJobId;
}
...
- (NSArray<DBTEAMMemberAddResult *> *)complete {
    if (![self isComplete]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBTEAMMembersAddLaunchComplete, but was %@.", [self tagName]];
    }
    return _complete;
}
