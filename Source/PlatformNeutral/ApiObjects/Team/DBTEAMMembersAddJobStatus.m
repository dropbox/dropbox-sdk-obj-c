...
- (NSArray<DBTEAMMemberAddResult *> *)complete {
    if (![self isComplete]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBTEAMMembersAddJobStatusComplete, but was %@.", [self tagName]];
    }
    return _complete;
}
...
- (NSString *)failed {
    if (![self isFailed]) {
        [NSException raise:@"IllegalStateException" format:@"Invalid tag: required DBTEAMMembersAddJobStatusFailed, but was %@.", [self tagName]];
    }
    return _failed;
}
...
+ (NSDictionary *)serialize:(DBTEAMMembersAddJobStatus *)valueObj {
    ...
    if ([valueObj isInProgress]) {
        ...
        jsonDict[@"complete"] = [DBArraySerializer serialize:valueObj.complete withBlock:^id(id elem) { return [DBTEAMMemberAddResultSerializer serialize:elem]; }];
...
+ (DBTEAMMembersAddJobStatus *)deserialize:(NSDictionary *)valueDict {
    ...
    if ([tag isEqualToString:@"in_progress"]) {
        ...
        NSArray<DBTEAMMemberAddResult *> *complete = [DBArraySerializer deserialize:valueDict[@"complete"] withBlock:^id(id elem) { return [DBTEAMMemberAddResultSerializer deserialize:elem]; }];
