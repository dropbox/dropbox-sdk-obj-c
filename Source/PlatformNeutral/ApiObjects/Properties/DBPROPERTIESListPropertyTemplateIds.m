...
- (instancetype)initWithTemplateIds:(NSArray<NSString *> *)templateIds {
    [DBStoneValidators arrayValidator:nil maxItems:nil itemValidator:[DBStoneValidators stringValidator:@(1) maxLength:nil pattern:@"(/|ptid:).*"]](templateIds);
...
+ (NSDictionary *)serialize:(DBPROPERTIESListPropertyTemplateIds *)valueObj {
    ...
    jsonDict[@"template_ids"] = [DBArraySerializer serialize:valueObj.templateIds withBlock:^id(id elem) { return elem; }];

    return jsonDict;
}

+ (DBPROPERTIESListPropertyTemplateIds *)deserialize:(NSDictionary *)valueDict {
    NSArray<NSString *> *templateIds = [DBArraySerializer deserialize:valueDict[@"template_ids"] withBlock:^id(id elem) { return elem; }];
