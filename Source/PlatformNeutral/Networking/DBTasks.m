...
- (DBError *)getDBError:(NSData *)data response:(NSURLResponse *)response error:(NSError *)error statusCode:(int)statusCode httpHeaders:(NSDictionary *)httpHeaders {
    ...
    if (statusCode >= 500 && statusCode < 600) {
        ...
        dbxError = [[DBError alloc] initAsAuthError:requestId statusCode:@(statusCode) errorContent:errorContent structuredAuthError:authError];
        ...
        dbxError = [[DBError alloc] initAsRateLimitError:requestId statusCode:@(statusCode) errorContent:errorContent structuredRateLimitError:rateLimitError backoff:@(retryAfterSeconds)];
...
- (instancetype)initWithTask:(NSURLSessionDataTask *)task session:(NSURLSession *)session delegate:(DBDelegate *)delegate route:(DBRoute *)route {
    self = [self init];
    if (self) {
        _task = task;
        _session = session;
...
- (DBRpcTask *)response:(void (^)(id, id, DBError *))responseBlock {
    void (^handler)(NSData *, NSURLResponse *, NSError *) = ^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        DBError *dbxError = [self getDBError:data response:response error:error statusCode:statusCode httpHeaders:httpHeaders];
        if (dbxError) {
...
- (instancetype)initWithTask:(NSURLSessionUploadTask *)task session:(NSURLSession *)session delegate:(DBDelegate *)delegate route:(DBRoute *)route {
    self = [self init];
    if (self) {
        _task = task;
        _session = session;
...
- (instancetype)initWithTask:(NSURLSessionDownloadTask *)task session:(NSURLSession *)session delegate:(DBDelegate *)delegate route:(DBRoute *)route overwrite:(BOOL)overwrite destination:(NSURL *)destination {
    self = [self init];
    if (self) {
        _task = task;
        _session = session;
...
- (instancetype)initWithTask:(NSURLSessionDownloadTask *)task session:(NSURLSession *)session delegate:(DBDelegate *)delegate route:(DBRoute *)route {
    self = [self init];
    if (self) {
        _task = task;
        _session = session;
