...
- (DBRpcTask *)requestRpc:(DBRoute *)route arg:(id<DBSerializable>)arg {
    ...
    NSDictionary *headers = [self headersWithRouteInfo:route.attrs[@"style"] serializedArg:serializedArg host:route.attrs[@"host"]];
...
- (DBUploadTask *)requestUpload:(DBRoute *)route arg:(id<DBSerializable>)arg inputURL:(NSURL *)input {
    ...
    DBUploadTask *uploadTask = [[DBUploadTask alloc] initWithTask:task session:_backgroundSession delegate:_delegate route:route];
    [task resume];
...
- (DBDownloadURLTask *)requestDownload:(DBRoute *)route arg:(id<DBSerializable>)arg overwrite:(BOOL)overwrite destination:(NSURL *)destination {
    NSURL *requestUrl = [self getUrl:route];
    NSString *serializedArg = [[self class] serializeArgString:route routeArg:arg];
    NSDictionary *headers = [self headersWithRouteInfo:route.attrs[@"style"] serializedArg:serializedArg host:route.attrs[@"host"]];
    ...
    DBDownloadURLTask *downloadTask = [[DBDownloadURLTask alloc] initWithTask:task session:_backgroundSession delegate:_delegate route:route overwrite:overwrite destination:destination];
    [task resume];
...
- (DBDownloadDataTask *)requestDownload:(DBRoute *)route arg:(id<DBSerializable>)arg {
    ...
    NSURLSessionDownloadTask *task = [_backgroundSession downloadTaskWithRequest:request];
    DBDownloadDataTask *downloadTask = [[DBDownloadDataTask alloc] initWithTask:task session:_backgroundSession delegate:_delegate route:route];
    [task resume];
...
+ (NSURLRequest *)requestWithHeaders:(NSDictionary *)httpHeaders url:(NSURL *)url content:(NSData *)content stream:(NSInputStream *)stream {
...
- (NSDictionary *)headersWithRouteInfo:(NSString *)routeStyle serializedArg:(NSString *)serializedArg host:(NSString *)host {
